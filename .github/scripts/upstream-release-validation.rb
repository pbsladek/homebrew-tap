...
ScriptHelpers.fail!("Invalid TAP_NAME: #{tap_name}") unless tap_name.match?(%r{\A[a-zA-Z0-9_.-]+/[a-zA-Z0-9_.-]+\z})
...
unless version.match?(/\Av[0-9]+\.[0-9]+\.[0-9]+([-.][0-9A-Za-z]+)*\z/)
  ScriptHelpers.fail!("Invalid version format: #{version}")
end
...
if run_brew_checks
  ...
  ScriptHelpers.run_cmd!('brew', 'untap', '--force', tap_name) if stdout.lines.map(&:strip).include?(tap_name)
